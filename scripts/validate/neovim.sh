...
config_source="$repo_root/dotfiles/common/.config/nvim"

if rg -n '/nix/store|/run/current-system|/etc/profiles/per-user|nix-plugin-loader|nix-darwin|home-manager' "$config_source"; then
  printf 'error: removed runtime path remains in Neovim configuration\n' >&2
  exit 1
fi
...
nvim --headless -l "$repo_root/scripts/validate/neovim.lua"
