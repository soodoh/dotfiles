...
import { STORE_SCHEMA_VERSION, type PersistedUsageState, type SessionStorageContext, type SuggestionUsageStatsPair } from "./session-state-types.js";

export class SessionUsageLedger {
	private readonly usageTasks = new Map<string, Promise<void>>();

	public async load(context: SessionStorageContext): Promise<SuggestionUsageStatsPair> {
		const persisted = await readJsonIfExists<PersistedUsageState>(context.usageFile!);
		if (!persisted) return emptyUsagePair();
		return normalizePersistedUsagePair(persisted);
	}

	public async record(context: SessionStorageContext, kind: "suggester" | "seeder", usage: SuggestionUsage): Promise<void> {
