...
export function renderSeedTrace(events) {
    if (events.length === 0) {
    ...
    return [
        ...
        "- log file: ~/.local/state/pi/pi-prompt-suggester/projects/<project>/logs/events.ndjson",
        ...lines,
    ].join("\n");
}
export function renderStatus(seed, state, config, ctx, activeVariantName) {
    ...
    return [
        ...
        `- ${compactUsageLine}`,
        `- logs: ${logFile} (use /suggester seed-trace)`,
