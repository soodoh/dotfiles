...
export async function handleSettingsUiCommand(ctx, composition) {
    ...
    const strategyOptions = ["compact", "transcript-steering"];
    const suggestionDisplayOptions = ["ghost", "widget"];
    ...
    const pickMenuAction = async () => {
        const items = [
            ...
            },
            {
                ...
                description: await describeScopedValue("suggestion.transcriptMaxContextPercent", composition.config.suggestion.transcriptMaxContextPercent),
            },
            {
                value: "suggestion.transcriptMaxMessages",
                label: "Transcript max messages",
                description: await describeScopedValue("suggestion.transcriptMaxMessages", composition.config.suggestion.transcriptMaxMessages),
            },
            {
                value: "suggestion.transcriptMaxChars",
                label: "Transcript max chars",
                description: await describeScopedValue("suggestion.transcriptMaxChars", composition.config.suggestion.transcriptMaxChars),
            },
            {
                value: "suggestion.transcriptRolloutPercent",
                label: "Transcript rollout %",
                description: await describeScopedValue("suggestion.transcriptRolloutPercent", composition.config.suggestion.transcriptRolloutPercent),
            },
            {
            ...
            },
            {
                ...
                description: await describeScopedValue("suggestion.displayMode", composition.config.suggestion.displayMode),
            },
            {
            ...
            },
            {
                ...
                description: await describeScopedValue("suggestion.showPanelStatus", composition.config.suggestion.showPanelStatus),
            },
            {
                value: "suggestion.showUsageInPanel",
                label: "Show suggester usage in panel",
                description: await describeScopedValue("suggestion.showUsageInPanel", composition.config.suggestion.showUsageInPanel),
            },
            {
    ...
    const promptPercent = async (label, currentValue, options) => {
        const raw = await ctx.ui.editor(label, String(currentValue));
        if (raw === undefined)
            return undefined;
        const parsed = Number.parseInt(raw.trim(), 10);
    ...
    while (true) {
        ...
        try {
            ...
            if (action === "suggestion.strategy") {
                ...
                const selected = await ctx.ui.select(`Suggestion strategy (${formatScopeName(activeScope)}, current: ${currentValue})`, [...strategyOptions]);
                if (!selected)
                    continue;
            ...
            if (action === "suggestion.displayMode") {
                const currentValue = await getScopedEditorValue("suggestion.displayMode", composition.config.suggestion.displayMode);
                const selected = await ctx.ui.select(`Show suggestions via which surface? (${formatScopeName(activeScope)}, current: ${currentValue})`, [...suggestionDisplayOptions]);
                if (!selected || !isSuggestionDisplayMode(selected))
                    continue;
                await persistence.writeValue(activeScope, action, selected);
                ctx.ui.notify(usesGhostEditor(selected)
            ...
            if (action === "suggestion.showPanelStatus") {
                const currentValue = await getScopedEditorValue("suggestion.showPanelStatus", composition.config.suggestion.showPanelStatus);
                const selected = await ctx.ui.select(`Show suggestion status in panel? (${formatScopeName(activeScope)}, current: ${currentValue ? "true" : "false"})`, ["true", "false"]);
                if (!selected)
                    continue;
                await persistence.writeValue(activeScope, action, selected === "true");
                ctx.ui.notify(`Updated ${action} in ${activeScope} override.`, "info");
            ...
            if (action === "suggestion.showUsageInPanel") {
                const currentValue = await getScopedEditorValue("suggestion.showUsageInPanel", composition.config.suggestion.showUsageInPanel);
                const selected = await ctx.ui.select(`Show suggester usage in panel? (${formatScopeName(activeScope)}, current: ${currentValue ? "true" : "false"})`, ["true", "false"]);
                if (!selected)
                    continue;
                await persistence.writeValue(activeScope, action, selected === "true");
                ctx.ui.notify(`Updated ${action} in ${activeScope} override.`, "info");
            ...
            if (action === "suggestion.transcriptMaxContextPercent") {
                const next = await promptPercent(`Transcript max context % (${formatScopeName(activeScope)})`, await getScopedEditorValue("suggestion.transcriptMaxContextPercent", composition.config.suggestion.transcriptMaxContextPercent), { allowZero: false });
                if (next === undefined)
                    continue;
                await persistence.writeValue(activeScope, action, next);
                ctx.ui.notify(`Updated ${action} in ${activeScope} override.`, "info");
            ...
            if (action === "suggestion.transcriptMaxMessages") {
                const next = await promptPositiveInt(`Transcript max messages (${formatScopeName(activeScope)})`, await getScopedEditorValue("suggestion.transcriptMaxMessages", composition.config.suggestion.transcriptMaxMessages));
                if (next === undefined)
                    continue;
                await persistence.writeValue(activeScope, action, next);
                ctx.ui.notify(`Updated ${action} in ${activeScope} override.`, "info");
            ...
            if (action === "suggestion.transcriptMaxChars") {
                const next = await promptPositiveInt(`Transcript max chars (${formatScopeName(activeScope)})`, await getScopedEditorValue("suggestion.transcriptMaxChars", composition.config.suggestion.transcriptMaxChars));
                if (next === undefined)
                    continue;
                await persistence.writeValue(activeScope, action, next);
                ctx.ui.notify(`Updated ${action} in ${activeScope} override.`, "info");
            ...
            if (action === "suggestion.transcriptRolloutPercent") {
                const next = await promptPercent(`Transcript rollout % (${formatScopeName(activeScope)}; 0 disables transcript steering)`, await getScopedEditorValue("suggestion.transcriptRolloutPercent", composition.config.suggestion.transcriptRolloutPercent), { allowZero: true });
                if (next === undefined)
                    continue;
                await persistence.writeValue(activeScope, action, next);
                ctx.ui.notify(`Updated ${action} in ${activeScope} override.`, "info");
