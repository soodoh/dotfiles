...
export async function createAppComposition(pi, cwd = process.cwd()) {
    ...
    const variantStore = new SuggesterVariantStore(stateDir);
    ...
    const eventLog = new NdjsonEventLog(path.join(stateDir, "logs", "events.ndjson"));
    ...
    const stateStore = new SessionStateStore(stateDir, () => runtimeRef.getContext()?.sessionManager);
