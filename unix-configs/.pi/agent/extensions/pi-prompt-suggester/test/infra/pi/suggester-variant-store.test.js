...
test("variant store applies active variant overrides to effective config", async () => {
	const stateDir = await mkdtemp(path.join(os.tmpdir(), "pi-suggester-variants-"));
	const store = new SuggesterVariantStore(stateDir);
	await store.init();
...
test("variant store normalizes invalid files and preserves default variant", async () => {
	...
	await writeFile(
		path.join(stateDir, "variants.json"),
		JSON.stringify({
...
test("variant store writes files under the provided project state directory", async () => {
	const stateDir = await mkdtemp(path.join(os.tmpdir(), "pi-suggester-variants-"));
	const store = new SuggesterVariantStore(stateDir);
	await store.init();
	await store.createVariant("alt");
	...
	await access(path.join(stateDir, "variants.json"));
	await access(path.join(stateDir, "ab-results.ndjson"));
});
