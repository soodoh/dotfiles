...
- Ghost suggestion accept keys support `space`, `right`, and `enter`; accept-only defaults to `right`, and accept-and-send defaults to `enter` via `suggestion.ghostAcceptAndSendKeys`.
