...
- A completed successful Heimdall stage (or matching build when stage evidence is unavailable), followed by the second clean thread scan, permits the final done-condition check.
...
When new review comments appear during the wait, repeat Steps 2–5 immediately; do not wait for unrelated pipeline stages to finish before addressing them. After the Heimdall stage reaches a terminal state (or the build when stage evidence is unavailable), re-fetch the PR threads and status for the current head and honor the final publication-lag scan from Step 5. Repeat Steps 2–5 whenever the review produced new unresolved Heimdall comments or a pushed fix triggered a new Heimdall run.

Finish only when all of these are true:

- the active branch still points to the intended PR;
- the latest relevant Heimdall code-review stage for the current PR head is terminal (or the matching build is terminal when stage evidence is unavailable), the post-terminal thread scan is clean, and there is no unaddressed job or infrastructure failure (or the user has explicitly accepted that failure);
