...
set -euo pipefail
cd "$(dirname "$0")"
version="${1:-$(npm view @earendil-works/pi-coding-agent version)}"
jq --arg version "$version" '.version = $version | .dependencies["@earendil-works/pi-coding-agent"] = $version' package.json > package.json.tmp
