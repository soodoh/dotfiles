...
if command -v brew >/dev/null 2>&1; then
  brew_formulae="$(brew leaves 2>/dev/null | json_lines)"
  brew_casks="$(brew list --cask 2>/dev/null | json_lines)"
  brew_taps="$(brew tap 2>/dev/null | json_lines)"
  if [ "$(printf '%s' "$brew_casks" | jq length)" -gt 0 ]; then
    mapfile -t installed_casks < <(printf '%s' "$brew_casks" | jq -r '.[]')
    brew_info="$(brew info --cask --json=v2 "${installed_casks[@]}" 2>/dev/null || printf '{"casks":[]}')"
  fi
fi
...
if [ -x "$legacy_npm" ]; then npm_globals="$($legacy_npm ls -g --depth=0 --json 2>/dev/null | jq '[.dependencies // {} | keys[]]' || printf '[]')"; fi
if [ -x "$legacy_bun" ]; then bun_globals="$($legacy_bun pm ls -g 2>/dev/null | sed -E '1d; s/^[^[:space:]]+[[:space:]]+//; s/@[^@[:space:]]+$//' | json_lines)"; fi
if [ -x "$legacy_cargo" ]; then cargo_globals="$($legacy_cargo install --list 2>/dev/null | awk '/^[^ ]/ {sub(/ v.*/, ""); print}' | json_lines)"; fi
if [ -x "$legacy_uv" ]; then uv_globals="$($legacy_uv tool list 2>/dev/null | awk '/^[^ -]/ {print $1}' | json_lines)"; fi
