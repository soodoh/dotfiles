...
for command_name in $replacement_commands; do
  ...
  if [ -z "$path" ] || [[ "$path" != /nix/store/* ]]; then smoke_ok=false; fi
done
...
casks="$(printf '%s' "$audit_json" | jq --argjson approved "$approved_casks" '[.observed.homebrew.casks[]? | select(. as $item | $approved | index($item) | not)]')"
...
if command -v brew >/dev/null 2>&1; then
  printf '%s' "$formulae" | jq -r '.[]' | while IFS= read -r item; do [ -n "$item" ] && brew uninstall --formula "$item"; done
  printf '%s' "$casks" | jq -r '.[]' | while IFS= read -r item; do [ -n "$item" ] && brew uninstall --cask "$item"; done
  printf '%s' "$taps" | jq -r '.[]' | while IFS= read -r item; do [ -n "$item" ] && brew untap --force "$item"; done
fi
...
[ -x "$legacy_npm" ] && printf '%s' "$globals" | jq -r '.npm[]' | while read -r item; do "$legacy_npm" uninstall -g "$item"; done || true
[ -x "$legacy_bun" ] && printf '%s' "$globals" | jq -r '.bun[]' | while read -r item; do "$legacy_bun" remove -g "$item"; done || true
