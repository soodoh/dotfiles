...
- Provider usage is best-effort, only runs when the `provider_usage` section is configured, and reflects every supported provider Pi reports as authenticated—not just the active model's provider. Supported providers include LLM Hub, Anthropic (OAuth), OpenAI Codex (subscription), GitHub Copilot, Google Gemini CLI / Antigravity (OAuth), Google Vertex AI (OAuth or ADC / gcloud), and OpenRouter (API key). LLM Hub is treated as a normal authenticated Pi provider: its endpoint comes from Pi's configured model/provider data and its API key is resolved through Pi's provider-auth APIs. Google Vertex delegates ADC discovery to `google-auth-library` so authorized-user, service-account, external-account, impersonated, and metadata-server credentials share one project/token resolution path. The project and location saved by `/login google-vertex` in `auth.json` are resolved through Pi's provider-auth APIs, so interactive Pi and the standalone Sketchybar CLI use the same stored configuration without requiring ambient Google environment variables. API-key-only Vertex configurations require an accompanying ADC identity because Cloud Monitoring quota data requires IAM authorization. It queries Cloud Code PA user quotas first, then uses Cloud Monitoring quota-exceeded and request-count metrics without issuing a concurrent request burst. The request-count path retains the existing 200 RPM estimate; reported usage/limit ratios remain a fallback when request-count retrieval fails.
