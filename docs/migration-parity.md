...
| AeroSpace config/startup | 2, 3, 5 | missing-only Homebrew fallback because its tap lacks mise cask API metadata; linked config and launch agent task |
| SketchyBar config/startup after AeroSpace | 2, 3, 5 | missing-only Homebrew fallback because its tap lacks mise formula API metadata; linked config and launch agent that waits for AeroSpace |
| JankyBorders colors/width/logs | 2, 5 | missing-only Homebrew fallback because its tap lacks mise formula API metadata; launch agent task |
